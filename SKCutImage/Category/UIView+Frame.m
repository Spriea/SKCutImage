...
- (void)sk_setViewShadowColor:(UIColor *)color offSize:(CGSize)size shadowRadius:(CGFloat)radius{
...
- (void)sk_setBackgroundGradual:(UIColor *)fromColor toColor:(UIColor *)toColor isHorizontal:(BOOL)isHorizontal{
    CAGradientLayer *grad1 = [CAGradientLayer layer];
    ...
    if (isHorizontal) {
        ...
        grad1.startPoint = CGPointMake(0, 0);
        grad1.endPoint = CGPointMake(0, 1.0);
    }
    
    grad1.frame = self.bounds;
    grad1.cornerRadius = self.layer.cornerRadius;
    [self.layer insertSublayer:grad1 atIndex:0];
}

- (void)sk_BeautiBackgroundGradual:(UIColor *)fromColor toColor:(UIColor *)toColor isHorizontal:(BOOL)isHorizontal{
    CAGradientLayer *grad1 = [CAGradientLayer layer];
    grad1.colors = @[(__bridge id)fromColor.CGColor, (__bridge id)toColor.CGColor];
    grad1.locations = @[@0, @1.0];
    if (isHorizontal) {
...
+ (UIView *)sk_lineView:(CGRect)frame backColor:(UIColor *)color{
    UIView *tempV = [[UIView alloc] initWithFrame:frame];
